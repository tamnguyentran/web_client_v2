...
const UserList = () => {
  ...
  const [shouldOpenUpdatePasswordModal, setShouldOpenUpdatePasswordModal] =
    useState(false);
  ...
  const handleLock = (e) => {
    ...
    setControlTimeOutKey(
      serviceInfo.LOCK.reqFunct + "|" + JSON.stringify(inputParam)
    );
    sendRequest(
      serviceInfo.LOCK,
      inputParam,
      handleResultLockLogin,
      true,
      handleTimeOut
    );
  };
  ...
  const handleUpdatePassword = () => {
    const inputParam = [
      modalUpdatePassword.o_2,
      modalUpdatePassword.o_5,
      newPassword,
    ];
    setProcessing(true);
    setControlTimeOutKey(
      serviceInfo.UPDATE_PASSWORD.reqFunct + "|" + JSON.stringify(inputParam)
    );
    sendRequest(
      serviceInfo.UPDATE_PASSWORD,
      inputParam,
      handleResultUpdatePassword,
      true,
      handleTimeOut
    );
  };
