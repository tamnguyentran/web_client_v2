...
const Permission = ({}) => {
  ...
  useEffect(() => {
    if (!!IDUser && IDUser !== "") {
      ...
      sendRequest(
        serviceInfo.GET_PERMISSION,
        inputParam,
        handleResultGetPermission,
        true,
        handleTimeOut
      );
    }
  ...
  const handleChange = (item, name, checked) => {
    ...
    sendRequest(
      serviceInfo.UPDATE_PERMISSION,
      inputParam,
      handleResultUpdatePermission,
      true,
      handleTimeOut
    );
    // setListPermission({})
  ...
  return (
    <div className="">
      ...
      <Card className="mb-2">
        <CardHeader
          title={
            <div className="flex aligh-item-center">{t("menu.list-permission")}</div>
          }
        />
        <CardContent>
          <Grid xs={12} className="mt-1 mb-1">
            <FormGroup className="w-100 permission-group">
              {Object.keys(listPermission).length > 0
                ? Object.values(listPermission)?.map((item, index) => (
                    <Accordion
                      expanded={expanded === item.parent_cd}
                      onChange={handleChangeAccordion(item.parent_cd)}
                    >
                      <AccordionSummary
                      ...
                      <AccordionDetails>
                        <Typography className="w-100">
                          {item?.result?.map((control, indexControl) => (
                            <>
                              <FormControl
                                className="d-flex permission"
                                key={control.scrn_cd + indexControl}
                                component="fieldset"
                              >
                                <FormGroup row>
                                  <FormControlLabel
                                    className="title-screen ml-2"
                                    control={<span />}
                                    label={control.scrn_nm}
                                  />
                                  <FormControlLabel
                                    control={
                                      <Checkbox
                                        disabled={control.ins_yn}
                              ...
                              {item?.result.length > indexControl + 1 && (
                                <Divider
                                  key={control.scrn_cd + indexControl}
