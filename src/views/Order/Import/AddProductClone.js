...
import { Unit, Product, AddCustomer } from "../../../components/Autocomplete";
...
const AddProduct = ({ onAddProduct, resetFlag, style, saveProcess }) => {
  ...
  const handleResultGetProductImportInfo = (reqInfoMap, message) => {
    if (message["PROC_STATUS"] !== 1) {
      ...
      setRequireExpDate(
        glb_sv.defaultProductGroupId.includes(data.rows[0]["o_4"])
      );
      if (data.rowTotal > 1) {
  ...
  const handleQuantityChange = (e) => {
    ...
    const newProductInfo = { ...productInfo };
    newProductInfo["qty"] = Number(glb_sv.filterNumber(e.target.value)) >= 0 ? Math.round(glb_sv.filterNumber(e.target.value)) : 0;
    setProductInfo(newProductInfo);
  };
  ...
  const handlePriceChange = (e) => {
    ...
    newProductInfo["price"] =
      Number(glb_sv.filterNumber(e.target.value)) >= 0 ? Math.round(glb_sv.filterNumber(e.target.value)) : 0;
    setProductInfo(newProductInfo);
  };
  ...
  const handleDiscountChange = (e) => {
    ...
    newProductInfo["discount_per"] =
      Number(glb_sv.filterNumber(e.target.value)) >= 0 && Number(glb_sv.filterNumber(e.target.value)) < 100
  ...
  const handleVATChange = (obj) => {
    ...
    newProductInfo["vat_per"] =
      Number(obj.value) >= 0 && Number(obj.value) <= 100
  ...
  return (
      ...
      <div style={{ borderBottom: "1px solid var(--gray3)", padding: "10px" }}>
        <div className="flex">
          <SelectCpn
            ...
            name="imp_tp"
          >
            <MenuItem value="1">{t("Nhập mua")}</MenuItem>
            <MenuItem value="2">{t("Hàng khuyến mãi")}</MenuItem>
          </SelectCpn>
          <Product
            className="mr-2"
            autoFocus={true}
          ...
          <TextFieldCpn
            ...
            onKeyPress={(event) => {
              if (event.key === "Enter") {
                let LotNo = productImportInfoData.find(
                  (x) => x.o_2 === productInfo.lot_no
                );
                if (!!LotNo) {
                  const newProductInfo = { ...productInfo };
                  newProductInfo["exp_dt"] = moment(
                    LotNo.o_3,
                    "YYYYMMDD"
                  ).toString();
                  setProductInfo(newProductInfo);
        ...
        <div className="flex align-items-end">
          <TextFieldCpn
            value={glb_sv.formatValue(productInfo.qty || 0, "currency")}
   ...
   {false && <Card className="mb-2" style={style}>
        <CardHeader  title={
