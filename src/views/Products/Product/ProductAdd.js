...
const ProductAdd = ({ onRefresh }) => {
  ...
  const [invoiceInventory, setInvoiceInventory] = useState({
    lot_no: "",
    exp_dt: null,
    imp_price: 0,
  });
  ...
  const handleCreate = async () => {
    ...
    if (product.store_current > 0) {
      if (
        invoiceInventory.lot_no != undefined &&
        invoiceInventory.lot_no.trim() !== ""
      ) {
      ...
      if (invoiceInventory.imp_price < 0) {
        SnackBarService.alert(
          t("product.inven_price_cannot_less_zero"),
          true,
          4,
          3000
        );
    ...
    if (storeLimit.minQuantity < 0 || storeLimit.maxQuantity < 0) {
      ...
      if (
        storeLimit.minQuantity >= storeLimit.maxQuantity &&
    ...
    const inputParam = [
      ...
      invoiceInventory.exp_dt
        ? moment(invoiceInventory.exp_dt).format("YYYYMMDD")
        : "",
      ...
      result.success ? result.data.image_nm : " "
    ];
  ...
  const handleSelectProductGroup = (obj) => {
    ...
    setRequireExpDate(
      !!obj ? glb_sv.defaultProductGroupId.includes(obj.o_2) : false
    );
    setProduct(newProduct);
  };
  ...
  const handleChangeRate = (obj) => {
    const newUnitRate = { ...unitRate };
    newUnitRate["rate"] =
      Number(obj.value) >= 0 ? Number(obj.value) : Math.abs(Number(obj.value));
    setUnitRate(newUnitRate);
  };
      ...
      <Dialog fullWidth={true} maxWidth="md" open={shouldOpenModal}>
        <Card>
          ...
          <CardContent className="cardContent_modal">
            ...
            <Divider orientation="horizontal" />
            <Accordion
              expanded={isExpandedInfo}
              onChange={handleChangeExpandInfo}
            >
              <AccordionSummary
            ...
            <Accordion
              expanded={isExpandedInfo2}
              onChange={handleChangeExpandInfo2}
            >
              <AccordionSummary
                expandIcon={<ExpandMoreIcon />}
            ...
            <Accordion
              expanded={isExpandedInfo3}
              onChange={handleChangeExpandInfo3}
            >
              <AccordionSummary
                expandIcon={<ExpandMoreIcon />}
