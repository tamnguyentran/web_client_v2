...
const StoreLimitEdit = ({
  ...
  useEffect(() => {
    if (shouldOpenModal && !!id && id !== 0) {
      setStoreLimit({});
      sendRequest(
        serviceInfo.GET_STORE_LIMIT_BY_ID,
        [id],
        handleResultGetStoreLimitByID,
        true,
        handleTimeOut
      );
    }
    ...
    >
      <Card>
        <CardHeader
          ...
          title={t("config.store_limit.titleEdit", { name: StoreLimit.o_3 })}
        />
        <CardContent>
          <Grid container spacing={1}>
            ...
            <Grid item xs={6}>
              <TextFieldCpn
                ...
                inputRef={step2Ref}
                value={glb_sv.formatValue(StoreLimit.o_6 || 0, "currency")}
                onKeyPress={(event) => {
                  if (event.key === "Enter") {
            ...
            <Grid item xs={6}>
              <TextFieldCpn
                ...
                onFocus={(e) => e.target.select()}
                inputRef={step3Ref}
                value={glb_sv.formatValue(StoreLimit.o_7 || 0, "currency")}
                onKeyPress={(event) => {
                  if (event.key === "Enter") {
                    handleUpdate();
                  }
