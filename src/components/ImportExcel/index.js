        ...
        fileReader.onload = (event) => {
            try {
                ...
                sheetNameList.forEach(function (y) {
                    ...
                    setUnitNotAvailable([
                        ...new Set(unitDataNameList.filter((item) => !unitSysNameList.includes(item))),
                    ])
                    setGroupNotAvailable([
                        ...new Set(groupDataNameList.filter((item) => !groupSysNameList.includes(item))),
                    ])
            ...
            columns.map((col) => {
                ...
                if (col.key === 'group') {
                    if (!item[col.key] || item[col.key].Trim() === '') {
                        ...
                        const findInd = info_dec.groups.findIndex((item) => item.name === item[col.key])
                        if (findInd < 0) {
                            item['validate'] = false
                            item['noted'] =
                                item['noted'].length > 0
                ...
                if (col.key === 'unit') {
                    if (!item[col.key] || item[col.key].Trim() === '') {
                        ...
                        const findInd = info_dec.units.findIndex((item) => item.name === item[col.key])
                        if (findInd < 0) {
                            item['validate'] = false
                            item['noted'] =
                                item['noted'].length > 0
                                    ? item['noted'] + ', ' + t('imp_excel.unit_min_info_uncorrect')
                                    : t('imp_excel.unit_min_info_uncorrect')
                        }
                    }
                }
                if (col.key === 'invenqty') {
                    if (!!item[col.key] && (isNaN(item[col.key]) || Number(item[col.key]) < 0)) {
                        item['validate'] = false
                        item['noted'] =
                            item['noted'].length > 0
                ...
                if (col.key === 'expire_date') {
                    if (!!item[col.key] || item[col.key].Trim() === '') {
                        if (
                            info_dec.groups.findIndex((item) => item['group'] === 'DƯỢC PHẨM') > 0 ||
                            info_dec.groups.findIndex((item) => item['group'] === 'THỰC PHẨM CHỨC NĂNG') > 0
                        ) {
                            item['validate'] = false
                            item['noted'] =
                                item['noted'].length > 0
                        ...
                        if (!arrDt || arrDt.length !== 3 || !glb_sv.verifyDt(arrDt[0], arrDt[1], arrDt[2])) {
                            item['validate'] = false
                            item['noted'] =
                                item['noted'].length > 0
            ...
            <Dialog fullWidth={true} maxWidth="md" open={shouldOpenModal}>
                <Card className="product-card">
                        ...
                        action={
                            <div className="d-flex align-items-center">
                                <Button
                                    ...
                                    onClick={() =>
                                        window.open(
                                            window.location.origin + '/asset/files/product_' + glb_sv.langCrt + '.xlsx',
                                            '_blank'
                                        )
                                    }
                                    disableElevation
                    ...
                    <CardContent>
                        ...
                        {isError && <Alert severity="error">{t('message.error_file')}</Alert>}
                        {dataSource.length > 0 && (
                    ...
                    <CardActions className="align-items-end" style={{ justifyContent: 'flex-end' }}>
                        ...
                        <Button
                            ...
                            className={dataSource && dataSource.length > 0 ? 'bg-success text-white' : ''}
                        >
                            {t('btn.save')} (F3)
                        </Button>
                    </CardActions>
                ...
                <Card className="product-card">
                    ...
                    <CardActions className="align-items-end" style={{ justifyContent: 'flex-end' }}>
                        ...
                        <Button
                            ...
                            className={
                                !editModal.name.trim() || !editModal.groupID || !editModal.unitID
                                    ? ''
