...
const Dictionary_Autocomplete = ({ diectionName, onSelect, label, style, size, value, disabled = false, placeholder=""}) => {
    ...
    return (
        <Autocomplete
            ...
            inputValue={value}
            style={style}
            renderInput={(params) => <TextField style={{fontSize:'10px'}} placeholder={placeholder} {...params} label={!!label ? label : ''} variant="outlined" />}
        />
    )
}
